    ...
    <main class="w-full">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div class="flex flex-wrap items-center gap-2 sm:gap-3">
          <button id="btnAddUser" class="h-10 px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-medium transition shadow flex items-center justify-center shrink-0">
            + Add User
          </button>
          <input id="searchInput" type="search" placeholder="Search users..." class="h-10 px-3 rounded-lg border border-slate-200 text-sm w-full sm:w-48 shrink-0" />
          <select id="filterUserType" class="h-10 px-3 rounded-lg border border-slate-200 text-sm min-w-[110px] shrink-0">
            <option value="all">All Types</option>
            <option value="internal">Internal</option>
            <option value="external">External</option>
          </select>
          <select id="filterRole" class="h-10 px-3 rounded-lg border border-slate-200 text-sm min-w-[110px] shrink-0">
            <option value="all">All Roles</option>
            <option value="customer">Customer</option>
            <option value="department_head">Department Head</option>
            <option value="admin">Admin</option>
          </select>
          <select id="filterStatus" class="h-10 px-3 rounded-lg border border-slate-200 text-sm min-w-[110px] shrink-0">
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
      ...
      <form id="userForm" class="p-6 space-y-4">
        ...
        <div id="passwordField">
          <label for="userPassword" class="block text-sm font-medium text-gray-700 mb-1">Password * <span class="text-gray-400 font-normal">(min 4 chars)</span></label>
        ...
        <div>
          <label for="userDepartment" class="block text-sm font-medium text-gray-700 mb-1">Department</label>
          <select id="userDepartment" name="department_id" class="w-full px-3 py-2 rounded-lg border border-slate-200 h-10">
            <option value="">— None —</option>
          </select>
        </div>
        <div>
          <label for="userCompany" class="block text-sm font-medium text-gray-700 mb-1">Company</label>
        ...
        <div class="flex justify-end gap-3 pt-4 border-t mt-6 items-center">
          <button type="button" id="btnCancel" class="px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 h-10 min-w-[80px] flex items-center justify-center">Cancel</button>
          <button type="submit" id="btnSave" class="px-4 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 h-10 min-w-[80px] flex items-center justify-center">Save</button>
        </div>
      </form>
    </div>
  </div>
  ...
  <div id="deleteModal" class="fixed inset-0 bg-black/40 flex items-center justify-center hidden z-50">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6">
      ...
      <div class="flex justify-end gap-3 items-center">
        <button id="btnDeleteCancel" class="px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 h-10 min-w-[80px] flex items-center justify-center">Cancel</button>
        <button id="btnDeleteConfirm" class="px-4 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700 h-10 min-w-[90px] flex items-center justify-center">Deactivate</button>
      </div>
    </div>
  </div>
