document.addEventListener("DOMContentLoaded", () => {
  ...
  sendBtn.addEventListener("click", async () => {
    ...
    if (file) formData.append("replyAttachment", file);

    try {
      const response = await fetch("../php/post_reply.php", {
        method: "POST",
        body: formData
      });
...
async function loadReplies(ref) {

  if (isLoading) return; // Prevent calling it again while it's loading
  isLoading = true;
  try {
    ...
    try {
      data = JSON.parse(text);
      console.log("Replies fetched:", data.replies?.length || 0, "entries");
    } catch (e) {
    ...
    // Ok for somereason there are duplicate replies being generated, This filter them out
    data.replies = data.replies.filter((value, index, self) => 
      index === self.findIndex((t) => (
        t.reply_id === value.reply_id
      ))
    );
